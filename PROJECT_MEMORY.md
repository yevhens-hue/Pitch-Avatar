...
### 2026-06-28 — Coach (Train Mode): Отображение картинок слайдов
- **Что сделано**: В `TrainModeUI` и `PracticePlayerUI` добавлена поддержка отображения реальных изображений слайдов.
- **Детали**: Если у объекта слайда (из базы или JSON) есть поле `image_url`, компонент выводит тег `<img>`. Если поля нет — выводится прежняя текстовая CSS-заглушка с заголовком и текстом. Это убирает разрыв между MVP и реальным дизайном, если слайды уже содержат картинки (например, загруженные через API).
...
---
### 2026-07-01 — Встреча с разработчиками: Архитектура Sara (виджет онбординга)

**Участники:** Евген, Юрий, Павло, Костянтин, Андрій, Олексій

**Ключевые технические решения (зафиксировать!):**

1. **Чипсы (Suggested Chips) — ИСКЛЮЧЕНЫ из MVP.** В ответах аватара будет только текст + текстовые гиперссылки. Клик на ссылку → `OpenURL` с `useNavigate` → открытие нужной страницы/раздела.

2. **Навигация (OpenURL + useNavigate):** Виджет может переходить на любую страницу приложения через уникальный URL. Открытие модальных окон (`modal`) — требует дополнительного прокидывания ID в код (пока не реализовано). Первый шаг MVP — только переходы по URL.

3. **Контекст страницы:** При переходе пользователя на новый URL — фронт передаёт виджету текущий URL + контекст страницы (описание Надії). Этот контекст добавляется в промт LLM при каждом запросе.

4. **Коммуникация CMS → виджет (postMessage / WebSocket):** Когда в приложении происходит ошибка или событие — CMS может передать информацию в виджет через `postMessage` (фронтовый API). Виджет поднимает тригер, тот обрабатывает данные и подсказывает пользователю следующий шаг.

5. **Система промтов (Костянтин):** Текущая система требует доработки — большой контекст страницы нельзя сразу вкидывать в prompt (LLM будет галлюцинировать). Предложение: хранить описания страниц в knowledge base (параметры/storage), подгружать чанками в рантайме.

6. **Архитектура хранилища страниц:** Описания страниц хранятся не в RAG и не в приложении, а в отдельном storage (аналог media data / слайдов в CMS). Тригеры используют эти данные для понимания контекста.

7. **MVP НЕ включает:** управление кнопками за пользователя, ввод данных в поля, открытие модалок без ID, история ачивментов пользователя (это следующий шаг).

**Итог встречи — что нужно сделать:**
- Описать каждую страницу как слайд (делает Надія) ✅ уже описаны
- Добавить Action `OpenURL` с `useNavigate` — уже работает ✅
- Добавить передачу текущего URL из фронта в виджет как параметр — нужно доделать
- Настроить `postMessage` API для передачи ошибок из CMS в виджет — нужно доделать
- Переработать систему промтов под чанковую подгрузку контекста (Костянтин)

**Сохранённые артефакты:**
- Расшифровка встречи: [`docs/sara/sara_meeting_transcript.md`](file:///Users/yevhen/.gemini/antigravity/scratch/projects/github/Pitch-Avatar/docs/sara/sara_meeting_transcript.md)
- Задача на дизайн (обновлена): [`docs/sara/sara_design_task.md`](file:///Users/yevhen/.gemini/antigravity/scratch/projects/github/Pitch-Avatar/docs/sara/sara_design_task.md)

---
### 2026-07-01 — Задача на дизайн Sara (обновлена после встречи с разработчиками)

**Главное изменение:** Убраны чипсы (Suggested Chips). Акцент — на дизайн кликабельных текстовых гиперссылок внутри бабблов бота.

**Ключевые блоки задачи:**
- **Header:** Context Title (динамический), Sound Toggle, ⋯ меню (Manage Attachments + Clear Chat), Close (X). Кнопка «минус» — убрана.
- **Avatar Area:** Состояния `idle / thinking / speaking` + статусный текст.
- **Chat Interface:** Бабблы (бот слева, пользователь справа). Акцент — стилизация текстовых ссылок как кликабельных элементов (цвет, подчёркивание).
- **Input Panel:** Микрофон (Continuous Voice с пульсацией) + текстовое поле (`Ask Sara anything...`) + кнопка отправки.
- **Proactive Bubbles:** FAB + всплывающий баббл для Time-based / Error-based / Success-based триггеров.
- **Mobile Fallback:** Виджет сворачивается в FAB при старте интерактивного тура.

**Визуальный референс:** Готовый виджет на [pitchavatar.com](https://pitchavatar.com/) — от него отталкиваться по стилю и layout.

**Название задачи (рекомендуется):** `[Design] Интерфейс ИИ-ассистента «Sara» (MVP)`
