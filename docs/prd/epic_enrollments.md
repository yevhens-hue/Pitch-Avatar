...
## 5. Architectural & Business Logic Updates (Post-PO Review)

> [!IMPORTANT]
> Эти правила были утверждены Product Owner-ом и переопределяют более ранние технические упрощения (от 2026-06-11).

### 5.1. Модель данных: Группы и связи (1:N)
*   **Группы (Group Enrollments)**: Один объект Enrollment может содержать множество уникальных ссылок/слушателей (связь `1:N`). В UI это выглядит как "раскрывающаяся строка" (Expandable Row), где родительский Enrollment объединяет дочерних слушателей. Архитектура должна поддерживать промежуточную таблицу (например, `enrollment_items` или возвращать pivot).

### 5.2. Обновление контента (Replace / Update Link)
*   Пользователь **может** изменить презентацию (Project) или курс (Course) в уже созданном Enrollment (через действие "Update" в меню таблицы).
*   При редактировании система подменяет (replace) внутренние `screenings` (снепшоты). Публичные ссылки продолжают работать, но показывают уже обновленный контент. Риск расхождения контента с уже отправленными email берет на себя пользователь.

### 5.3. Механика статусов
*   **Статус Failed**: Обозначает исключительно системную или биллинговую ошибку (например, "не хватило кредитов квоты" или "сбой генерации"). Он **не** используется для фиксации того, что слушатель пропустил просмотр.
*   **Анонимные слушатели (Anonymous)**: Так как анонимную ссылку могут открыть несколько разных людей, статус всего Enrollment агрегируется по "максимальному" результату. (Если хоть одна сессия завершилась просмотром — статус `Completed`).
