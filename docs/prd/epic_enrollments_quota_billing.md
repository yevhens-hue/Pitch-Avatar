...
*   **Acceptance Criteria**:
    *   В разделе "Enrollments" (или в глобальном хедере) постоянно отображается индикатор (прогресс-бар или текст), показывающий отношение занятых мест к общему лимиту (например, "85/100 Enrollments in use").
