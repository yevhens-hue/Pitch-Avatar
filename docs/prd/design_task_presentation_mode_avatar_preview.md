...
1. **В Галерее Шаблонов (`/presentation-templates`):**
   ...
   - В выпадающем фильтре типов отсутствовали точные опции (`All Types`, `Presentation + AI Avatar`, `AI Avatar Only`, `Presentation`).
...
* **1. Галерея Шаблонов (Templates UI):** Метки типов `Presentation` / `Presentation + Avatar`, выпадающий фильтр по типам (`All Types`, `Presentation + AI Avatar`, `AI Avatar Only`, `Presentation`), карточка аватара/голоса в модале предпросмотра слайдов и бесшовный переход по `Use template`.
...
* **3. Пользовательский интерфейс (User Workspace & Access Modal):**
  ...
  * Полное описание сквозной логики расшаривания прав от пользователя к суперадминистратору.
...
#### 1.3. Карточка Аватара и Голоса в модальном окне `Preview slides`
...
#### 3.2. 🔄 Логика взаимодействия и передачи прав доступа

1. **Действие пользователя (User UI — `app.pitchavatar.com`):**
   * Пользователь заходит в свои проекты (**My Projects**).
   * Выбирает **Actions ⚙️ ➔ Access** у нужной презентации.
   * В попапе **Access Modal** выбирает пункт **`Available to Superadmin`** и сохраняет.

2. **Изменение статуса и прав (Backend / State):**
   * Проекту присваивается тип доступа `accessType: 'superadmin'` (и флаг `isShared: true`).
   * Презентация становится автоматически доступной для аккаунта суперадминистратора.

3. **Отображение у Суперадмина (Super Admin UI — `admin.pitchavatar.com`):**
   * В таблице проектов суперадмина презентация появляется на вкладке **`Shared with me`** (Проекты, расшаренные пользователями).
   * Суперадмин видит автора презентации, дату расшаривания и метку доступа `Available to Superadmin`.

4. **Что может делать Суперадмин с расшаренным проектом:**
   * **Помощь и проверка:** Открыть, просмотреть слайды, проверить настройки ИИ-аватара и озвучки.
   * **Создание Шаблонов (Templates):** При создании системного шаблона в разделе **«Добавить шаблон»** этот проект станет доступен для выбора в таблице **«Исходный проект»** (со всеми имеющимися слайдами и аватаром).
...
- [ ] **Галерея Шаблонов (Templates):**
  ...
  - [ ] Отрисован выпадающий фильтр со списком вариантов (`All Types`, `Presentation + AI Avatar`, `AI Avatar Only`, `Presentation`).
...
- [ ] **Пользовательский интерфейс (User UI):**
  ...
  - [ ] Зафиксирована и согласована логика передачи прав от пользователя суперадминистратору.
