...
*   **Trainer / Тренер (Експерт):** Адміністратор, що налаштовує систему в *Train Mode*. Тренер бачить, як аватар генерує та задає питання, і "на льоту" коригує ідеальні відповіді, навчаючи систему (збереження в Knowledge Base/RAG).

**Ключові особливості MVP:**
*   **Проактивність ШІ:** Аватар завжди ініціює діалог та запитує першим. (Сценарій, де учень сам ставить питання для виявлення потреб, відкладено на майбутнє).
*   **Гнучкість слайдів:** Запитання-відповіді є сутністю Knowledge Base. Тренування можна почати з будь-якого етапу. Використання слайдів як візуального аргументу — опціональне, хоча і корисне для деяких клієнтів (напр. Softprom).
*   **Project-level активація:** Режим тренування (Coach Mode) — це налаштування на рівні всього проекту (відображається спеціальною іконкою "гантельки" у списку проектів).
